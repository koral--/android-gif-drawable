...
static int DGifGetWord(GifFileType *GifFile, uint_fast16_t *Word);
...
******************************************************************************/
static int
DGifGetWord(GifFileType *GifFile, uint_fast16_t *Word) {
    unsigned char c[2];
    ...
    *Word = (uint_fast16_t) UNSIGNED_LITTLE_ENDIAN(c[0], c[1]);
    return GIF_OK;
}
